...
df['revenues'] = df['revenues'].str.replace('[\$,]', '', regex=True).astype(int)
df['profits'] = df['profits'].str.replace('[\$,]', '', regex=True).astype(float).astype(float)
df['assets'] = df['assets'].str.replace('[\$,]', '', regex=True).astype(int)
df['marketvalue'] = df['marketvalue'].str.replace('[\$,]', '', regex=True).astype(float)

# Removing percentage signs and converting to float, then to integer
df['revenuechange'] = df['revenuechange'].str.replace('%', '').astype(float)
df['profitschange'] = df['profitschange'].str.replace('%', '').astype(float)
df.head()
...
print(df['revenues'].max() - df['revenues'].min())
...
plt.boxplot(df['revenues'], vert=False)
...
skewness_revenues = df['revenues'].skew()
skewness_revenues
...
kurtosis_revenues = df['revenues'].kurt()
kurtosis_revenues
...
value_counts_revenues = df['revenues'].value_counts()
value_counts_revenues
